...
def client():
    ...
    # # Mocking redis_memory
    # redis_memory = MagicMock(aioredis.Redis)

    # # Manually initialize FastAPILimiter with redis_memory
    # FastAPILimiter.redis = redis_memory
...
def app_without_limiter():
    app = FastAPI()
    # Ініціалізація лімітера у події startup (може бути винесено в окремий модуль)
    @app.on_event("startup")
    async def startup_event():
        pass  # Залишаємо ініціалізацію лімітера порожнею для відключення лімітера
