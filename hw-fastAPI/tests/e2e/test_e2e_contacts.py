...
def test_create_contact(client, get_token, monkeypatch):
    with patch.object(auth_service, 'cache') as redis_mock:
        ...
        response = client.post("/api/contacts/", headers={"Authorization": f"Bearer {get_token[0]}"},
                                json={**contact_dict, "user": "deadpool@example.com", "avatar": "127.0.0.1://my_avatar.jpg"},)
        # Assertions
    ...
    with patch.object(auth_service, 'cache') as redis_mock:
        ...
        response_payload = [ContactResponseSchema(**{"id": 2, "first_name": "John", "last_name": "Bon Jovi",
                            ...
                            ContactResponseSchema(**{"id": 4, "first_name": "Lenny", "last_name": "Wolf",
                                                    ...
                                                    "created_at": None, "updated_at": None}),]
                            # ContactResponseSchema(**{"id": 3, "first_name": "Nana", "last_name": "Kwame",
                            #                          "email": "darkman@germany.eu", "phone_number": "0373335500",
                            #                          "birth_date": date(1968, 10, 5), "crm_status": "operational"}),]
        
        mock_search_contact_by_birthdate = AsyncMock(request_payload)
        monkeypatch.setattr("src.routes.birthday_contacts.search_contact_by_birthdate", mock_search_contact_by_birthdate)
...
async def test_search_contact_complex(client, get_token, monkeypatch):
    with patch.object(auth_service, 'cache') as redis_mock:
        ...
        async with TestingSessionLocal() as session:
            attrb = "ohn"
            queries = await rep_contacts.search_contact_complex(attrb, session)
